...
#### Q1: 
I tried looking for commands with password in them but they had to big of a char count. 

I then looked up for a command with passwd in it. I found some commands and after looking online the one that was working.

q1 flag: `procdump64.exe -ma lsass.exe passwd.txt`

#### Q2: 
I searched for the word Users in the file and found a lot of commands, however the one that stood out was the one that did not have Users as part of a file path. I then looked on google `enumerate users windows command` and got this website from microsoft: https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/net-user
...
#### Q4: 
After searching for the word Desktop we find the command 2 commands, since we are on windows the one with `dir` in it is the one we care about.
